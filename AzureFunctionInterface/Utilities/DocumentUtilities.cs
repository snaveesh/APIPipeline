...
namespace AzureFunctionInterface.Utilities
{
    ...
    {
        public static DocumentClient GetDocumentClient(string endPointUri, string primaryKey)
        {
            return new DocumentClient(new Uri(endPointUri), primaryKey);
        }

        public static async Task AddDocumentAsync(string endpointUri, string primaryKey, string database, string container, string partitionKey, string storedProc, Dictionary<string, string> customer)
        {
            var client = GetDocumentClient(endpointUri, primaryKey);
        ...
        public static async Task<string> ReadDocumentsbyPartition(string endpointUri, string primaryKey, string database, string container, string partitionKey, string storedProc, string partitionValue)
        {
            var client = GetDocumentClient(endpointUri, primaryKey);
            await CreateStoredProcedureIfNotExists(client, database, container, storedProc, partitionKey);
